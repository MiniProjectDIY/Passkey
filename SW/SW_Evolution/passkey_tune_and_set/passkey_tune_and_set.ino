...
void command_use(void) {
  ...
  DebugSerial.println("\"4\" means \"display chip sn.\"");
  ...
  if (DebugSerial.available() > 0) {
    ...
    if (command == "1") {
      ...
      if (register_FP()) {
        ...
        DebugSerial.println("FP registration process failed! Please 'register' again!!!");
      }
    } else if (command == "2") {
      //搜索指纹匹配
      ...
      if (search_FP()) {
        ...
        DebugSerial.println("FP match failed! Please 'search' again!!!");
      }
    } else if (command == "3") {
      //清除模组存储的指纹
      ...
      if (clear_all_fingerprint()) {
        ...
        DebugSerial.println("FP clear FP all lib failed! Please 'clear' again!!!");
      }
    } else if (command == "4") {
      //读取指纹模组的SN
      DebugSerial.println("[INFO] reading module serial number...");
      if (get_module_sn(module_sn)) {
        DebugSerial.println("Module SN:");
        for (int i = 0; i < 16; i++) {
